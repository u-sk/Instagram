...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    // Firebaseのクラスを保存するプロパティとPostDataクラスの配列
    ...
    override func viewDidLoad() {
        ...
        // 「PostTableViewCell.xib」ファイルに作成したセルの定義内容をUINib(nibName:bundle)で取得
        let nib = UINib(nibName: "PostTableViewCell", bundle: nil)
        // Outletで接続しているTableViewにregister(_:forCellReuseIdentifier:)メソッドで登録
        tableView.register(nib, forCellReuseIdentifier: "Cell")
        
        // UITableView.automaticDimensionは、テーブル行の高さをAuto Layoutに任せるという設定
        // テーブル行の高さをAutoLayoutで自動調整する
        tableView.rowHeight = UITableView.automaticDimension
        // UITableView.automaticDimensionを設定した場合、estimatedRowHeightプロパティに行の高さの見積もり値を設定しておく必要があります。
        // テーブル行の高さの概算値を設定しておく
